...
2. **Load preset configuration**:
   - Read **vendor defaults** from `.specify/presets/taskstoissues-jira/config/jira.yml`. Expected keys: `default_issue_type`, `labels_default`.

     > [!CAUTION]
     > If the file does not exist, abort with: `> .specify/presets/taskstoissues-jira/config/jira.yml not found. Reinstall the taskstoissues-jira preset before re-running.`

   - Read **consumer-side config** from `dev/jira.yml` at the repo root. Required keys: `cloud`, `default_project_key`, `custom_fields.epic_link`, `custom_fields.team`. Optional keys (override vendor defaults if present): `default_issue_type`, `labels_default`, additional `custom_fields.*` (e.g. `sprint`, `story_points`).

     > [!CAUTION]
     > If `dev/jira.yml` does not exist, abort with: `> dev/jira.yml not found. Copy .specify/presets/taskstoissues-jira/templates/jira.example.yml to dev/jira.yml at the repo root, fill in cloud + default_project_key + custom_fields.{epic_link,team}, and commit before re-running.`
     ...
     > [!CAUTION]
     > If `cloud` still equals the example placeholder `your-tenant.atlassian.net`, abort with: `> dev/jira.yml still has placeholder cloud 'your-tenant.atlassian.net'. Set it to your Atlassian site hostname before re-running.`
   ...
   - **Merge**: build the runtime config by overlaying `dev/jira.yml` on top of the vendor file per top-level key (consumer wins). After merge, the effective config must include `cloud`, `default_project_key`, `default_issue_type`, `custom_fields.epic_link`, `custom_fields.team`, `labels_default`.
...
3. **Resolve Atlassian cloud id**: Call `mcp__claude_ai_Atlassian__getAccessibleAtlassianResources` once and match `cloud` from the merged config against the returned site URLs to obtain `cloudId`.
...
8. **Create one Jira issue per phase**: For each parsed phase, call `mcp__claude_ai_Atlassian__createJiraIssue` with:
   ...
   - `additional_fields`:
     ...
     - `labels`: union of `labels_default` (merged config) + `labels` (per-user override) + `US<story>` if the phase has a `story` value
     - `custom_fields`:
